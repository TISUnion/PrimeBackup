...
class RestoreBackupTask(HeavyTask[None]):
	...
	def __countdown_and_stop_server(self, backup: BackupInfo) -> bool:
		for countdown in range(max(0, self.config.restore.countdown_sec), 0, -1):
	...
	def run(self):
		...
		try:
			...
			if self.config.restore.create_pre_restore_backup and restore_path.exists():
			...
			self.logger.info('Restoring to backup #{} at path {!r} (fail_soft={}, verify_blob={})'.format(
				backup.id, restore_path.as_posix(), self.fail_soft, self.verify_blob,
			))
