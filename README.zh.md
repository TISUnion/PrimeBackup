...
- 打包文件用于存储压缩后的二进制条目，例如数据块载荷，降低小文件压力，同时支持校验和整理
...
文件会被切分成由内容定义的数据块（chunk），每个数据块都会计算哈希值；如果数据块的内容没有改变，它就可以在不同的备份中被复用，只有新的数据块载荷会作为打包条目写入存储

Prime Backup 支持常见的文件类型，包括普通文件、目录和符号链接；对于这三类文件：

- 普通文件：Prime Backup 会先计算其哈希值（及文件大小）
  启用 CDC 时，文件以“chunked blob”形式存储，并引用多个数据块；数据块会独立去重，新的数据块载荷会被压缩并存储为打包条目
