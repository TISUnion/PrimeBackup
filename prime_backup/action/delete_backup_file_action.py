...
class DeleteBackupFileAction(Action[BlobDeltaSummary]):
	...
	def run(self) -> BlobDeltaSummary:
		...
		self.logger.info('Deleting file {!r} in backup #{}, removed {} blobs, {} chunks and changed {} packs (freed disk {} / raw {})'.format(
			self.file_path, self.backup_id,
			bds.blob_count, bds.chunk_count, bds.packs.changed_pack_count, ByteCount(bds.freed_disk_size).auto_str(), ByteCount(bds.raw_size).auto_str(),
		))
		return bds
