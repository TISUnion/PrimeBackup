...
1. [备份目标](config.zh.md#targets)，即你需要备份的那些文件夹和文件。
    你需要把 `targets` 数组里的 `"world"` 修改成你的存档文件夹的名字
    ...
    除此之外，如果你使用的是像 Bukkit 那样将每个维度存储于独立文件夹的服务端，你可能需要这么配置：
...
2. 用于计算/储存备份数据的方式
    ...
    !!! note
        ...
        建议你从一开始就慎重且明智地设置这两个选项

        虽然你依然可以在 MCDR 中使用 `!!pb database migrate_xxx` 指令来迁移现有备份的压缩方法和哈希算法，
