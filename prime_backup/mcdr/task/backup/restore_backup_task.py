...
class RestoreBackupTask(HeavyTask[None]):
	...
	def run(self):
		...
		failures = ExportBackupToDirectoryAction(backup.id, self.config.source_path, delete_existing=True, fail_soft=self.fail_soft, verify_blob=self.verify_blob).run()
