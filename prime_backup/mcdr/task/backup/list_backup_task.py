...
class ListBackupTask(LightTask[None]):
	def __init__(self, source: CommandSource, per_page: int, page: int, backup_filter: BackupFilter, show_all: bool, show_flags: bool):
		...
		if not self.show_all:
			self.backup_filter.requires_non_temporary_backup()
			self.backup_filter.requires_non_hidden_backup()
	...
	def __make_command(self, page: int) -> str:
		def date_str(ts_ns: int) -> str:
			return json.dumps(conversion_utils.timestamp_to_local_date_str_ns(ts_ns, decimal=ts_ns % (10 ** 9) != 0), ensure_ascii=False)
		...
		if self.backup_filter.timestamp_ns_start is not None:
			cmd += f' --from {date_str(self.backup_filter.timestamp_ns_start)}'
		if self.backup_filter.timestamp_ns_end is not None:
			cmd += f' --to {date_str(self.backup_filter.timestamp_ns_end)}'
		if self.show_all:
			cmd += f' --all'
		if self.show_flags:
			cmd += f' --flags'
