...
class ShowDbOverviewTask(LightTask[None]):
	...
	def run(self) -> None:
		...
		self.reply(self.tr('db_file_size', make_size(result.db_file_size)))
		...
		self.reply(self.tr('file_raw_size', make_size(result.file_raw_size_sum)))
		self.reply(self.tr('blob_count', TextComponents.number(result.blob_count)))
		self.reply(self.tr('blob_stored_size', make_size(result.blob_stored_size_sum), TextComponents.percent(result.blob_stored_size_sum, result.blob_raw_size_sum)))
		self.reply(self.tr('blob_raw_size', make_size(result.blob_raw_size_sum)))
