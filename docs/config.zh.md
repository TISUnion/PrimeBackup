...
一个 [gitignore 风格](http://git-scm.com/docs/gitignore) 的模板串列表，
与 [creation_skip_missing_file](#creation_skip_missing_file) 选项配合使用
...
一个 [gitignore 风格](http://git-scm.com/docs/gitignore) 的模板串列表，
匹配这些模式的文件将在创建备份时被视为"频繁变化文件"

对于此类文件，Prime Backup 将：

- 跳过预计算哈希：在主备份流程开始前，不会对这些文件预先计算哈希值
- 总是先拷贝到临时文件再处理：不直接读取原始文件，而是先将其复制到临时位置，再对临时副本进行处理。这可以保证即使文件正在被并发写入，也能获取到一份稳定的文件内容快照

此选项适用于那些在执行 [`/save-off`](#commands) 命令后，依然随时会发生变化的文件（例如模组数据库、日志文件等），
可加快备份过程中处理这些文件的速度

!!! note

    此选项仅影响 Prime Backup 创建备份时处理这些文件的策略，不控制文件是否被纳入备份。
    若想将某文件完全排除在备份之外，请使用 [ignore_patterns](#ignore_patterns) 或 [retain_patterns](#retain_patterns)
...
- 对大于 10 MiB 的 `.log` 文件启用 `fixed_128k` 固定大小分块
- 对大于 256 KiB 的 `.mca` 文件启用 `fixed_auto` 分块
...
子配置 `compact`、`backup` 和 `compact_pack` 描述了与数据库和打包文件相关的定时作业
...
它会使用 [`backup.pack_maintenance_compact_threshold`](#pack_maintenance_compact_threshold) 作为存活数据阈值整理打包文件
