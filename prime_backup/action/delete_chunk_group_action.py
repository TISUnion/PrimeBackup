...
class DeleteChunkGroupsAction(Action[ChunkGroupListSummary]):
	...
	def __collect_chunk_group_ids_to_delete(self, session: DbSession) -> List[int]:
		...
		chunk_groups_by_id: Dict[int, Optional[schema.ChunkGroup]] = session.get_chunk_groups_by_ids_opt(self.chunk_group_ids)
		...
		chunk_groups_by_hash: Dict[str, Optional[schema.ChunkGroup]] = session.get_chunk_groups_by_hashes_opt(self.chunk_group_hashes)
